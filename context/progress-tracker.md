...
**Last completed:** Special Events launch screen — a menu-launched screen (main-menu row 18) lists the scenario challenges with saved completion badges and spoken briefings/objectives, and running one drives the real race loop, scores/persists objectives, and returns to the list announcing the result. The special-event mode is now playable end to end. Prior: completion persistence; startup cue-coverage log; per-tier career reward scaling.
...
**Last verification:** `python -m pytest` — 525 passed, 100 subtests passed
...
- [x] Complete career progression with rewards, contracts, fatigue, upgrades, and stable consequences that affect future races. Rewards now scale with the season's escalating difficulty tier: `DifficultyTier` gained a `reward_multiplier` (Rookie 0.9x, Pro 1.0x, Elite 1.25x), `record_race_result` takes a `career_difficulty_id` and multiplies the rank-based `base_reward` by that tier (min 1), and `pygame_main` threads the actual `career_difficulty` tier of each race into the result so a hard Elite closer pays appreciably more than a Rookie opener. The result summary records `difficulty_tier`/`reward_multiplier` and the post-race feedback speaks a "Difficulty bonus" line. Backward compatible (no tier id → 1.0x). Combined with the already-shipped contract payouts, fatigue/injury consequences, rest recovery, and stable-upgrade effects on future races, career progression now has a complete reward/consequence loop. Covered by `tests/test_difficulty.py`. Still open: per-tier scaling of contract prizes and selectable difficulty for non-career modes.
...
- [x] Remove menu dead zones and ensure every screen has clear status, back/cancel, repeat/help, and failure feedback. Every pygame screen now honors `R = repeat` for the audio-first flow: the read-only Statistics and Career Result screens re-speak their summary on `R` instead of exiting on any key (`_handle_key`), and the main menu, career hub, profile, and online lobby re-announce the current selection/status on `R`. Back/cancel (Esc/Q/M), spoken status, and failure feedback were already present per screen. On-screen hints updated to list `R`, covered by repeat-key contract tests in `tests/test_pygame_secondary_screens.py` and `tests/test_pygame_menu_models.py`.
...
- [ ] Add more official tracks with distinct turn, surface, weather, and obstacle profiles. In progress: added `highcliff_rise` (Highcliff Rise) — a 1480 m left-handed turf circuit with a genuine elevation change (uphill climbing turn at 320–700 m, downhill recovery straight at 700–1100 m, uphill sweeping finish) and an 8-obstacle hill/cliff set (rockfall/scree stones, timber gate, rolling barrel, warning rope, spring seep, cliff rail, overhang) exercising all three dodge/jump/duck actions. Auto-selectable via `load_tracks`; passes the deterministic balance envelopes across all weathers (`tests/test_race_balance.py`) and has obstacle/content-load coverage. Still open: more tracks and surface variety.
- [x] Expand career length and calendar variety. The championship season grew from 6 to 9 races (`content/championship.json`), and career length auto-follows because it is data-driven from the calendar (`_career_cap`, `career_difficulty`, `career_title` all key off `len(calendar)` — no code change needed). New rounds add the `highcliff_rise` track to the career calendar (`Highcliff Ascent`, windy; `Highcliff Storm`, rain) and introduce the previously-unused `fog` weather to career play (`Fog Gauntlet` on Bracken Dirt), so the season now spans all four non-lab tracks and all four weathers with evolving rival-stable assignments. Covered by updated `tests/test_content_loading.py` and `tests/test_championship.py`; balance envelopes for every track/weather combo (including the new career pairings) still pass. Still open: contracts/calendar branching depth and even longer seasons.
- [x] Give rivals clearer identity through narrative hooks, spoken lines, behavior, and sound signatures. `RivalProfile` gained optional `racing_style` (behavior descriptor), `rivalry_hook` (narrative), `falling_behind_line`, and `blocking_line` fields, all authored for the six rivals in `content/rivals.json` (loader uses a new `_string_default` helper so the fields stay backward compatible). At race start `PygameRaceGame._announce_rivals` now speaks each opponent rival's `narrative_intro()` (name + style + hook) once — turning the previously-dead `intro_line` data into a real spoken field — and `_speak_rival_events` now also voices the `opponent_falling_behind` and `opponent_blocking_inside` events with per-rival lines (previously those events only produced generic audio). Rivals already carry per-horse `signature_sound` cues layered by the spatial audio system. Covered by domain/data tests in `tests/test_content_loading.py` and behavior tests in `tests/test_pygame_smoke.py`. Still open: distinct per-rival AI behavior in the sim and bespoke signature sound assets.
- [x] Add special events, scenarios, time trials, and ghost challenges that reuse the core race loop. Time Trial and Ghost Race were already playable; this adds a **special-event scenario layer** (`horse_racing_game/app/special_events.py`) that wraps the existing deterministic `GameApp.run_quick_race` loop in scored objectives — no new simulation. Ships a catalog of three distinct challenges over existing content (Fog Sprint Gauntlet on Bracken Dirt/fog with finish+time-limit+podium objectives; Highcliff Endurance Climb with a stamina-reserve objective; Ashford Champion Charge with a win objective), each built on the pre-existing `RaceEventSpec`/`ScenarioObjective` model so specs validate against the shared `scenario` racing mode. `observed_values_for`/`evaluate_special_event` score a finished `RaceState` into completed objectives (every metric framed as `value >= target`), `special_event_summary` produces spoken/readable feedback, and `run_special_event` plus the `--smoke-special-event` entrypoint run and score the full catalog headlessly. Per-challenge completion now persists in its own `save/special_events.json` file (mirroring the social/community/track-catalog aux-save pattern rather than threading fragile `GameProgress` fields): `record_special_event_result` keeps the best objectives-met count, a sticky `completed` flag, and the best finishing time across attempts, and `load_special_event_records` restores them; the `--smoke-special-event` runner exercises the full run→score→persist→reload loop. Covered by `tests/test_special_events.py`. A menu-launched **Special Events** screen (`ui/pygame_special_events.py`, main-menu row 18) now lists the challenges with saved completion badges, speaks each one's briefing/objectives/status (R repeats, Esc returns), and on selection `pygame_main._run_special_event_mode` runs the challenge on the real `PygameRaceGame` loop, scores it via `special_event_result_from_state`, persists with `record_special_event_result`, and returns to the list announcing the spoken result summary. Covered by `tests/test_pygame_secondary_screens.py` and `tests/test_pygame_menu_models.py`. This mode is fully playable end to end.
